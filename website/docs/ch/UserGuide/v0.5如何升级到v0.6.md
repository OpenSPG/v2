...
## 更新mysql数据库
KAG 在0.6版本增加了若干新的元数据表，如果用户想保留原有知识库的数据，需要手动往 mysql 容器中添加表；若否，则清理掉openspg 相关的本地镜像，重新拉取最新版镜像文件即可。
...
docker rmi spg-registry.cn-hangzhou.cr.aliyuncs.com/spg/openspg-server:latest
```

+ **拉取最新版本的 opengspg-server 并启动服务**
